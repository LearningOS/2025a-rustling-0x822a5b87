...
impl<T> LinkedList<T> {
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self where T: PartialOrd + Copy {
        let a_is_empty = list_a.length == 0;
        let b_is_empty = list_b.length == 0;
        match (a_is_empty, b_is_empty) {
            (true, true) => LinkedList::new(),
            (false, true) => list_a,
            (true, false) => list_b,
            (false, false) => {
                ...
                loop {
                    match(a_start, b_start) {
                        (None, None) => return new_list,
                        (Some(a_ptr), None) => {
                            let a_node = unsafe { a_ptr.as_ref() };
                            new_list.add(a_node.val);
                            a_start = a_node.next;
                        },
                        (None, Some(b_ptr)) => {
                            let b_node = unsafe { b_ptr.as_ref() };
                            new_list.add(b_node.val);
                            b_start = b_node.next;
                        },
